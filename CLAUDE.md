...
| `js/inspection-blank-forms.js` | **Blank printable field worksheets** — `buildBlankExtinguisherFormBytes()` / `buildBlankExitSignFormBytes()`. For contractors who fill out paper by hand: draws a clean numbered grid whose columns match the panels one-for-one (plain bordered cells, no form fields), reusing `drawReportHeader` for the cover (pre-fills the property when selected; Job/PO/Date/Inspector print blank). Dispatched by `buildActiveBlankFormBytes()` → `downloadBlankForm()` (js/inspection-main.js); triggered by the **🖨 Blank Field Form** button on the extinguisher/exit-sign panels. The controlled grid is deliberately what `inspection-scan-import.js` reads back. Loaded after `inspection-pdf-editable.js`. |
