...
| `js/inspection-blank-forms.js` | **Blank printable field worksheets** — `buildBlankExtinguisherFormBytes(opts)` / `buildBlankExitSignFormBytes(opts)`. For contractors who fill out paper by hand: each unit is a **two-row block** with open-text cells (location/type/floor/size/notes) plus **☐-to-X checkboxes** for every categorical field (mount, type, P/F/N-A columns, PASS/FAIL) — far more reliable for `inspection-scan-import.js` to read back. Reuses `drawReportHeader` for the cover (pre-fills property when selected; Job/PO/Date/Inspector print blank). `opts = { count=100, known/knownEL/knownES }`: `count` blank blocks per section (from the **Blank rows per section** input on step-1, capped 300), optionally preceded by devices pre-filled from the last inspection (**Pre-fill known devices** toggle → `_propertyProfile.lastInspBySystem[sys]`). Dispatched by `buildActiveBlankFormBytes(sys, opts)` → `downloadBlankForm(sys)` + `_blankFormOpts(sys)` (js/inspection-main.js); triggered from the **🖨 Blank Field Worksheets** card on the **step-1 start screen** (not the system panels). Loaded after `inspection-pdf-editable.js`. |
| `js/inspection-scan-import.js` | **Scan → autoload** — reads a photo/PDF of a hand-filled worksheet with Claude vision and populates the active panel's rows via the same `addExtUnitRow`/`addELRow`/`addESRow` prefill functions the wizard uses. Reuses the browser-side Anthropic pattern (`localStorage.flips_anth_key` + `anthropic-dangerous-direct-browser-access`); defaults to Sonnet (handwriting needs a strong vision model; override via `localStorage.flips_scan_model`). Triggered by **📷 Scan Paper Form** on the extinguisher/exit-sign panels; shows a review list, never auto-submits. The requested JSON keys MUST stay in sync with the panel prefill contracts (`SCAN_SYSTEMS`). Supports `extinguisher` and `exit-sign-lighting` only. |
