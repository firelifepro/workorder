...
| `admin.html` | **Admin Tools hub** — gated launcher (card grid) for the staff-only one-off tools: Price History Backfill, Legacy Cleanup, Import External Reports, Import Inspect Point, Sub Invoices, Potential Lease or Buy (`real-estate-agent.html`). Those pages are NOT in the shared nav; this hub is their only menu entry. Gate = `flipsIsAdmin()` (see `js/flips-nav.js`); non-admins are `location.replace`d to index.html. |
